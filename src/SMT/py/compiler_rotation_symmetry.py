...
if __name__ == "__main__":
    ...
    path_statistics = "rotation\\statistics\\statistics.csv"
    with (open(os.path.join(output_dir, path_statistics), 'w') as csv_):
        ...
        for instance in range(1, 41):
            ...
            height = max_custom([rot_y[i] + coord_y[i] for i in range(n_circuits)])
            ...
            for i in range(0, n_circuits):
                ...
                optimizer.add(If(rotation[i], And(rot_y[i] == x[i], rot_x[i] == y[i]),
                                 And(rot_y[i] == y[i], rot_x[i] == x[i])))
                optimizer.add(rot_x[i] + coord_x[i] <= plate)
                optimizer.add(rot_y[i] + coord_y[i] <= height)
            ...
            for i in range(0, n_circuits):
                for j in range(0, n_circuits):
                    if i != j:
                        optimizer.add(Or(
                            (coord_x[i] + rot_x[i] <= coord_x[j]), (coord_x[j] + rot_x[j] <= coord_x[i]),
                            (coord_y[i] + rot_y[i] <= coord_y[j]), (coord_y[j] + rot_y[j] <= coord_y[i])))

            # CONSTRAINT, use the cumulative to remove blank spaces and reduce time
            cumulative = []

            for z in y:
                cumulative.append(sum([If(And(coord_y[i] <= z, z < coord_y[i] + rot_y[i]), rot_x[i], 0)
            ...
            for i in range(n_circuits):
                for j in range(n_circuits):
                    if i < j:
                        optimizer.add(If(Or(And((rot_x[i] == rot_x[j]), (rot_y[i] == rot_y[j])),
                                            And((rot_x[i] == rot_y[j]), (rot_y[i] == rot_x[j]))),
                                         Or((coord_x[i] <= coord_x[j]), (coord_y[i] <= coord_y[j])),
                                         True))
            
            # CONSTRAINT, put the biggest circuit (maximum area) in (0,0)
            circuit_max_area = circuit_area.index(max(circuit_area))
            optimizer.add(coord_x[circuit_max_area] == 0)
            optimizer.add(coord_y[circuit_max_area] == 0)

            # CONSTRAINT, check if it's a square is useless to rotate and if rotated is larger than the plate
            for i in range(n_circuits):
                optimizer.add(If((x[i] == y[i]), (rotation[i] == False), True))
                optimizer.add(If((y[i] > plate), (rotation[i] == False), True))
